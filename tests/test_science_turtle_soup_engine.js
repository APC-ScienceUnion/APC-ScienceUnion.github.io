...
test("API 透传每日开局限额的上海重置时间与剩余额度", async (t) => {
  const originalFetch = global.fetch;
  global.fetch = async () => new Response(JSON.stringify({
    error: {
      code: "DAILY_GAME_LIMIT",
      message: "你今天已经开始过 3 局科学海龟汤，请明天再来。",
      retryable: false,
      retryAfterMs: 3600000,
      resetAt: "2026-08-21T16:00:00.000Z",
      remaining: 0,
      requestId: "request-daily-limit"
    }
  }), {
    status: 429,
    headers: { "Content-Type": "application/json" }
  ...
  t.after(() => { global.fetch = originalFetch; });
  await assert.rejects(
    () => Api.start("biology", "11111111-1111-4111-8111-111111111111"),
    (error) => error.status === 429
      && error.code === "DAILY_GAME_LIMIT"
      && error.retryable === false
      && error.retryAfterMs === 3600000
      && error.resetAt === "2026-08-21T16:00:00.000Z"
      && error.remaining === 0
  );
});

test("API 保留 425 可重试与 ACTION_FAILED 明确不可重试语义", async (t) => {
  const originalFetch = global.fetch;
  const replies = [
    {
      status: 425,
    ...
    },
    {
      ...
      error: {
        code: "ACTION_FAILED",
        message: "这次 AI 判断没有完成，请重新提交这条内容。",
        retryable: false
      }
    },
    {
  ...
  global.fetch = async () => {
    ...
    return new Response(JSON.stringify({ error: next.error }), {
      status: next.status,
      headers: { "Content-Type": "application/json" }
    });
  };
  ...
  await assert.rejects(
    () => Api.question(TOKEN, "它是动物吗？", 0, "11111111-1111-4111-8111-111111111111"),
    (error) => error.status === 425
      && error.code === "ACTION_IN_PROGRESS"
      && error.retryable === true
      && error.retryAfterMs === 3000
  );
  await assert.rejects(
    () => Api.question(TOKEN, "它是动物吗？", 0, "22222222-2222-4222-8222-222222222222"),
    (error) => error.status === 503
      && error.code === "ACTION_FAILED"
      && error.retryable === false
  );
  await assert.rejects(
    () => Api.question(TOKEN, "它是动物吗？", 0, "33333333-3333-4333-8333-333333333333"),
...
test("AI 前端展示标题、开局限额、隐私说明并按错误码选择正确重试路径", () => {
  const root = path.resolve(__dirname, "../source/science-turtle-soup");
  const index = fs.readFileSync(path.join(root, "index.html"), "utf8");
  const app = fs.readFileSync(path.join(root, "app.js"), "utf8");
  const setupStart = index.indexOf('<dialog id="setupDialog"');
  const setupEnd = index.indexOf("</dialog>", setupStart);
  const setup = index.slice(setupStart, setupEnd);

  assert.match(index, /<h1 id="pageTitle">科学海龟汤<\/h1>/);
  ...
  assert.match(app, /startRetryContext = \{ actionId, domainId \}/);
  assert.match(app, /startNewGame\(\{ \.\.\.context, skipConfirm: true \}\)/);
  assert.match(app, /error\.code === "ACTION_IN_PROGRESS"/);
  assert.match(app, /schedulePendingRetry\(error, pending\)/);
  assert.match(app, /const actionId = pending\.actionId/);
  assert.match(app, /\["ACTION_FAILED", "ACTION_PREVIOUSLY_FAILED"\]\.includes\(error\.code\)/);
  assert.match(app, /restorePendingDraft\(pending, true\)/);
  assert.ok(
    app.indexOf('["ACTION_FAILED", "ACTION_PREVIOUSLY_FAILED"]') < app.indexOf("error.status === 409"),
    "失败 actionId 必须先于通用 409 冲突处理"
  );
});
