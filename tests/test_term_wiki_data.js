...
assert.match(sharedApp, /expandedCategories: new Set\(config\.categories\.map\(category => category\.id\)\)/)
assert.doesNotMatch(sharedApp, /收起目录|展开目录|toggle-tree|toggleTree|treeCollapsed|treeCollapse|is-tree-collapsed/)
assert.doesNotMatch(sharedApp, /max-width:\s*760px/)
assert.match(sharedApp, /function toggleCategoryBranch \(button\)/)
assert.match(sharedApp, /list\.hidden = !expanding/)
assert.match(sharedApp, /expandedCategories\.add\(categoryId\)/)
assert.match(sharedApp, /expandedCategories\.delete\(categoryId\)/)
...
assert.doesNotMatch(sharedStyles, /tree-collapse|tree-toolbar|is-tree-collapsed/)
assert.match(sharedStyles, /\.term-wiki__tree-items\[hidden\]\s*{\s*display:\s*none;/)
...
assert.match(wikiLayout, /term-gallery-20260824-5/)
