...
(function attachScienceSoupApi(root, factory) {
  ...
  class ApiError extends Error {
    constructor(message, options) {
      ...
      this.resetAt = options
        && typeof options.resetAt === "string"
        && Number.isFinite(Date.parse(options.resetAt))
        ? options.resetAt
        : "";
      this.remaining = options
  ...
  function errorFromResponse(response, payload) {
    ...
    return new ApiError(message, {
      ...
      resetAt: source && typeof source.resetAt === "string" ? source.resetAt : "",
      remaining: source && Number.isSafeInteger(source.remaining) ? source.remaining : null
    });
  }
